...
struct Cubemap
{
	...
	Cubemap(int32_t w, int32_t h) : width(w), height(h), comp(3)
	{
	...
	glm::vec3 calculate_direction(Face face, int32_t face_x, int32_t face_y) const 
	{
		...
		switch (face) 
		{
		case POS_Z: x = s; y = -t; z = 1; break;
		case NEG_Z: x = -s; y = -t; z = -1; break;
		case NEG_X: x = -1; y = -t; z = s; break;
		case POS_X: x = 1; y = -t; z = -s; break;
		case POS_Y: x = s; y = 1; z = t; break;
		case NEG_Y: x = s; y = -1; z = -t; break;
		}
